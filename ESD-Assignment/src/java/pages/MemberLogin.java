    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (query[0].equals("")) {
            ...
            if (jdbc.retrieveStatus(query[0], query[1]).equals("APPROVED")) {
                double q  = jdbc.RetrieveFromMembers(query[0], "BALANCE", "MEMBERS");
                String clm = jdbc.RetrieveClaimsAndPayments(query[0],"CLAIMS","DATEOFPURCHASE","RATIONALE","AMOUNT");
                String pymnt = jdbc.RetrieveClaimsAndPayments(query[0],"PAYMENTS","TYPE_OF_PAYMENT","AMOUNT","DATEOFPURCHASE");
                ...
                request.getRequestDispatcher("/WEB-INF/MemberPage.jsp").forward(request, response);
            }
            else if (jdbc.retrieveStatus(query[0], query[1]).equals("APPLIED")){
                request.setAttribute("msg", "Member");
                request.getRequestDispatcher("/WEB-INF/NonMemberPage.jsp").forward(request, response);
            }
            else if (jdbc.retrieveStatus(query[0], query[1]).equals("SUSPENDED")){
                request.setAttribute("msg", "Member");
                request.getRequestDispatcher("/WEB-INF/suspendedPage.jsp").forward(request, response);
            }
            else {
