...
public class MemberServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        else if(request.getParameter("tbl").equals("Suspend")){
            dbBean.deactivate((String) session.getAttribute("nameofuser"));
            System.out.println((String) session.getAttribute("nameofuser"));
            request.getRequestDispatcher("index.jsp").forward(request, response);    
        }
        else if(request.getParameter("tbl").equals("Resume")){
            PrintWriter out = response.getWriter();
            dbBean.resumeUser((String) session.getAttribute("nameofuser"));
            System.out.println((String) session.getAttribute("nameofuser"));
            request.getRequestDispatcher("index.jsp").forward(request, response);   
            out.print("Account resumed");
            out.close();
        }

        else if (request.getParameter("tbl").equals("memPayments")){
            //request.setAttribute("queryPayments", msg);
            session.setAttribute("nameofuser", request.getParameter("username"));
        ...
        else if (request.getParameter("tbl").equals("payBalance")){
            //request.setAttribute("queryPayments", msg);
            request.getRequestDispatcher("/WEB-INF/memPayments.jsp").forward(request, response);   
        }
        else {
