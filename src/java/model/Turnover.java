...
public class Turnover extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String qryPayoutDetails = "select * from claims where STATUS='APPROVED' and DATEOFPURCHASE between '"+query+"-01-01' AND '"+query+"-12-31'";
        String qryPayout = "select SUM(amount) from claims where STATUS='APPROVED' and DATEOFPURCHASE between '"+query+"-01-01' AND '"+query+"-12-31'";
        
        Jdbc jdbc = (Jdbc)session.getAttribute("dbbean"); 
        // TESTING
        Date startQueryDate = java.sql.Date.valueOf(query+"-01-01");
        Date endQueryDate = java.sql.Date.valueOf(query+"-12-31");
        double sumAmount = jdbc.retrieveSumOfClaims(startQueryDate, endQueryDate);
        // TESTING
        if (jdbc == null)
            request.getRequestDispatcher("/WEB-INF/conErr.jsp").forward(request, response);
        
        if(qryIncome == null) {
            session.setAttribute("message", "cannot be NULL");
        } 
        else {
            ...
            session.setAttribute("queryIncomeDetails", msg1);
            session.setAttribute("queryIncome", msg2);
            session.setAttribute("queryPayoutDetails", msg3);
            session.setAttribute("queryPayout", msg4);
